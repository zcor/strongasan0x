...
Nothing you do in this skill is visible to the public: unpublished weeks render only for an
authenticated Django staff user and 404 for everyone else. The ingest step is idempotent under
`--overwrite`. If the operator lacks Django staff credentials, hand back the local Markdown
artifact for review rather than claiming the unpublished URL itself is a usable preview. Stop
before publishing.
...
[The legacy command](../../../rollcall/management/commands/publish_roll_call.py) is unmaintained since the 2026-02-14 initial
...
**CurveCap's submitted Telegram attestation is the source of record. Preserve it by default.**
Garmin detail and historical notes are diagnostic evidence, not permission to improve his
submission after the fact. Surface the following to the user before trials when relevant:

1. **Missing weights.** Garmin often retains exercise names and reps but not working loads. The
   Mon/Wed Y circuit pattern is machine-based, high-rep work in an approximate 49.5–110 lb station
   range; it is not proof of an individual set's load this week. Garmin may retain a maximum load
   for other sessions, but a maximum is not every set's load. Do not append either fact to
   `raw_text` unless the user explicitly authorizes an exception under the close-tie rule below.
2. **Sunday–Saturday reporting convention.** CurveCap intentionally uses this weekly window. It
   differs from the Monday–Sunday contest window but is not an off-by-one error. When a comparison
   is close, inspect and explain both boundary Sundays; do not silently re-window the submission or
   paste in the other Sunday.
3. **Zero-days from missed syncs.** `0` steps, `0` kcal, and `0 bpm` resting HR together cannot be
   a rest day and indicate missing data. Report them. Do not fill them from a later Garmin pull
   without the user's explicit authorization.

**Close-tie rule:** CurveCap is judged from exactly what he submitted in Telegram. Only when a
tie is genuinely unresolved may the user authorize changing his attestation, and then the change
may only favor the opponent. Never use a close tie to add evidence, weights, or activities that
improve CurveCap's standing. A clear numerical gap is not a tie.

The Garmin source tables live outside the Roll Call model. Review them separately when asked, and
keep their observations distinct from the submitted text. If a user-authorized edit legitimately
changes `raw_text`, delete the week's derived `RankingTrial` records and rerun; never mix trials
from two versions of an attestation.
...
resolve genuine ties; ranking order is their editorial call, not yours. For an unresolved tie that
includes CurveCap, apply the submitted-attestation and guest-favoring rule above; do not repair his
submission upward to break it.
...
Report the canonical preview URL — `https://strongasan0x.com/roll-call/<week-end>/` — only as a
staff-authenticated preview. When the operator has no Django staff session, instead link the local
`logs/<week-end>/substack_<week-end>.md` artifact for review. Summarize the final ranking order and
anything you corrected in the ode.
