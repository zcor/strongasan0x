...
class DashboardQueryTests(TestCase):
    ...
    @patch("daily.views.ensure_prior_day_coached")
    def test_ai_dashboard_applies_queued_work_without_calling_model(self, coach):
        participant = _make_participant(beta=True, ai_mutations_enabled=True)
        participant.onboarded_at = timezone.now()
        participant.save(update_fields=["onboarded_at"])
        version = ChecklistVersion.objects.create(
            participant=participant,
            questions=[{"key": "q_old", "label": "Old habit"}],
            source=ChecklistVersion.SOURCE_BASELINE,
            is_current=True,
        )
        ...
        suggestion = CoachSuggestion.objects.create(
            check_in=check_in,
            suggestion_text="A plan generated by the hourly coach.",
            proposed_questions=[{"key": "q_new", "label": "New habit"}],
            base_questions=version.questions,
            status=CoachSuggestion.STATUS_PENDING,
        )
        ...
        self.assertEqual(suggestion.status, CoachSuggestion.STATUS_APPLIED)
        current = participant.checklist_versions.get(is_current=True)
        self.assertEqual([item["key"] for item in current.questions], ["q_new"])
...
class PwaStartupTests(TestCase):
    def test_manifest_uses_climb_launch_colors(self):
        response = self.client.get("/daily/manifest.webmanifest")
