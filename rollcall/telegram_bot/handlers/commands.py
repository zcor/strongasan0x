...
async def handle_login_deeplink(update: Update, context: ContextTypes.DEFAULT_TYPE, token: str):
    ...
    if not bot_instance:
        await update.message.reply_text("Bot instance not available. Please try again.")
        return
...
async def view_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Handle /view command"""
    if not context.args:
        await update.message.reply_text("Usage: /view <username>")
        return
    ...
    try:
        ...
        await update.message.reply_text(view_message, parse_mode='Markdown')
    except Exception as e:
        logger.error(f"Error in view command: {e}")
        await update.message.reply_text("Error retrieving attestation. Please try again later.")
...
async def delete_part_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        await update.message.reply_text("Usage: /delete_part <part_number>")
        return
    
    try:
        part_number = int(context.args[0])
    except ValueError:
        await update.message.reply_text("Part number must be an integer")
        return
    ...
    if not attestation:
        await update.message.reply_text("No attestation found for current week")
        return
